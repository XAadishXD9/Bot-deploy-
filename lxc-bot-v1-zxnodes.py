...
logger = logging.getLogger('zxnodes_vps_bot')
...
# Embed creation functions with black theme and ZXNODES branding
def create_embed(title, description="", color=0x1a1a1a):
    """Create a dark-themed embed with proper field length handling and ZXNODES branding"""
    embed = discord.Embed(
        title=truncate_text(f"⭐ ZXNODES - {title}", 256),
        description=truncate_text(description, 4096),
        color=color
    )

    embed.set_thumbnail(url="https://i.imgur.com/xSsIERx.png")
    embed.set_footer(text=f"ZXNODES VPS Manager • {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
...
def is_admin():
    async def predicate(ctx):
        ...
        raise commands.CheckFailure(f"You need admin permissions to use this command. Contact ZXNODES support.")
...
async def get_or_create_vps_role(guild):
    ...
    role = discord.utils.get(guild.roles, name="ZXNODES VPS User")
    if role:
    ...
    try:
        role = await guild.create_role(
            name="ZXNODES VPS User",
            color=discord.Color.dark_purple(),
            reason="ZXNODES VPS User role for bot management",
            permissions=discord.Permissions.none()
        )
        VPS_USER_ROLE_ID = role.id
        logger.info(f"Created ZXNODES VPS User role: {role.name} (ID: {role.id})")
        return role
    except Exception as e:
        logger.error(f"Failed to create ZXNODES VPS User role: {e}")
        return None
...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="ZXNODES VPS Manager"))
    bot.loop.create_task(vps_monitor())
    logger.info("ZXNODES Bot is ready! VPS monitoring started.")
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        ...
        error_msg = str(error) if str(error) != "Admin required" else "You need admin permissions for this command. Contact ZXNODES support."
        ...
        await ctx.send(embed=create_error_embed("System Error", "An unexpected error occurred. ZXNODES support has been notified."))
...
async def ping(ctx):
    ...
    embed = create_success_embed("Pong!", f"ZXNODES Bot latency: {latency}ms")
...
async def my_vps(ctx):
    ...
    if not vps_list:
        await ctx.send(embed=create_embed("No VPS Found", "You don't have any ZXNODES VPS. Contact an admin to create one.", 0xff3366))
        return
    embed = create_info_embed("My ZXNODES VPS", "")
    text = []
...
async def lxc_list(ctx):
    ...
    try:
        ...
        embed = create_info_embed("ZXNODES LXC Containers List", result)
        await ctx.send(embed=embed)
    except Exception as e:
...
async def create_vps(ctx, ram: int, cpu: int, disk: int, user: discord.Member):
    ...
    container_name = f"zxnodes-vps-{user_id}-{vps_count}"
    ram_mb = ram * 1024

    await ctx.send(embed=create_info_embed("Creating ZXNODES VPS", f"Deploying VPS for {user.mention}..."))
    ...
    try:
        ...
        if ctx.guild:
            ...
            if vps_role:
                try:
                    await user.add_roles(vps_role, reason="ZXNODES VPS ownership granted")
                except discord.Forbidden:
                    logger.warning(f"Failed to assign ZXNODES VPS role to {user.name}")

        # Create success embed for channel
        embed = create_success_embed("ZXNODES VPS Created Successfully")
        ...
        try:
            dm_embed = create_success_embed("ZXNODES VPS Created!", f"Your VPS has been successfully deployed by an admin!")
            add_field(dm_embed, "VPS Details", f"**VPS ID:** #{vps_count}\n**Container Name:** `{container_name}`\n**Configuration:** {config_str}\n**Status:** Running\n**Created:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", False)
            add_field(dm_embed, "Management", "• Use `!manage` to start/stop/reinstall your ZXNODES VPS\n• Use `!manage` → SSH for terminal access\n• Contact ZXNODES admin for upgrades or issues", False)
            add_field(dm_embed, "Important Notes", "• Full root access via SSH\n• Ubuntu 22.04 pre-installed\n• Back up your data regularly with ZXNODES tools", False)
...
class ManageView(discord.ui.View):
        ...
        if len(vps_list) > 1:
            options = [
                discord.SelectOption(
                    label=f"ZXNODES VPS {i+1} ({v.get('config', 'Custom')})",
                    description=f"Status: {v.get('status', 'unknown')}",
                    value=str(i)
                ) for i, v in enumerate(vps_list)
            ]
            self.select = discord.ui.Select(placeholder="Select a ZXNODES VPS to manage", options=options)
            self.select.callback = self.select_vps
            self.add_item(self.select)
            self.initial_embed = create_embed("ZXNODES VPS Management", "Select a VPS from the dropdown menu below.", 0x1a1a1a)
    ...
    async def create_vps_embed(self, index):
        ...
        embed = create_embed(
            f"ZXNODES VPS Management - VPS {index + 1}",
        ...
        if suspended:
            add_field(embed, "⚠️ Suspended", "This ZXNODES VPS is suspended. Contact an admin to unsuspend.", False)

        live_stats = f"**CPU Usage:** {cpu_usage}\n**Memory:** {memory_usage}\n**Disk:** {disk_usage}"
        add_field(embed, "📈 Live Usage", live_stats, False)

        add_field(embed, "🎮 Controls", "Use the buttons below to manage your ZXNODES VPS", False)
    ...
    async def select_vps(self, interaction: discord.Interaction):
        if str(interaction.user.id) != self.user_id and not self.is_admin:
            await interaction.response.send_message(embed=create_error_embed("Access Denied", "This is not your ZXNODES VPS!"), ephemeral=True)
            return
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if suspended and not self.is_admin and action != 'stats':
            await interaction.response.send_message(embed=create_error_embed("Access Denied", "This ZXNODES VPS is suspended. Contact an admin to unsuspend."), ephemeral=True)
            return
        ...
        if action == 'stats':
            ...
            memory_usage = await get_container_memory(container_name)
            disk_usage = await get_container_disk(container_name)
            stats_embed = create_info_embed("📈 ZXNODES Live Statistics", f"Real-time stats for `{container_name}`")
        ...
        if action == 'reinstall':
            if self.is_shared or self.is_admin:
                await interaction.response.send_message(embed=create_error_embed("Access Denied", "Only the ZXNODES VPS owner can reinstall!"), ephemeral=True)
                return
            if suspended:
                await interaction.response.send_message(embed=create_error_embed("Cannot Reinstall", "Unsuspend the ZXNODES VPS first."), ephemeral=True)
                return

            confirm_embed = create_warning_embed("ZXNODES Reinstall Warning",
            ...
            class ConfirmView(discord.ui.View):
                ...
                async def confirm(self, interaction: discord.Interaction, item: discord.ui.Button):
                    ...
                    try:
                        ...
                        await interaction.followup.send(embed=create_info_embed("Recreating Container", f"Creating new ZXNODES container `{self.container_name}`..."), ephemeral=True)
                        ...
                        save_data()
                        await interaction.followup.send(embed=create_success_embed("Reinstall Complete", f"ZXNODES VPS `{self.container_name}` has been successfully reinstalled!"), ephemeral=True)
            ...
            try:
                ...
                vps["status"] = "running"
                save_data()
                await interaction.followup.send(embed=create_success_embed("VPS Started", f"ZXNODES VPS `{container_name}` is now running!"), ephemeral=True)
                new_embed = await self.create_vps_embed(self.selected_index)
            ...
            try:
                ...
                vps["status"] = "stopped"
                save_data()
                await interaction.followup.send(embed=create_success_embed("VPS Stopped", f"ZXNODES VPS `{container_name}` has been stopped!"), ephemeral=True)
                new_embed = await self.create_vps_embed(self.selected_index)
                await interaction.message.edit(embed=new_embed, view=self)
            except Exception as e:
            ...
            if suspended:
                await interaction.response.send_message(embed=create_error_embed("Access Denied", "Cannot access suspended ZXNODES VPS."), ephemeral=True)
                return
            await interaction.response.send_message(embed=create_info_embed("SSH Access", "Generating ZXNODES SSH connection..."), ephemeral=True)
            ...
            try:
                ...
                if check_proc.returncode != 0:
                    ...
                    await interaction.followup.send(embed=create_success_embed("Installed", "ZXNODES SSH service installed!"), ephemeral=True)

                # Start tmate with unique session name using timestamp
                session_name = f"zxnodes-session-{datetime.now().strftime('%Y%m%d%H%M%S')}"
                ...
                if ssh_url:
                    try:
                        ssh_embed = create_embed("🔑 ZXNODES SSH Access", f"SSH connection for VPS `{container_name}`:", 0x00ff88)
                        ...
                        await interaction.followup.send(embed=create_success_embed("SSH Sent", f"Check your DMs for ZXNODES SSH link! Session: {session_name}"), ephemeral=True)
                    except discord.Forbidden:
                        await interaction.followup.send(embed=create_error_embed("DM Failed", "Enable DMs to receive ZXNODES SSH link!"), ephemeral=True)
                else:
...
async def manage_vps(ctx, user: discord.Member = None):
    """Manage your ZXNODES VPS or another user's VPS (Admin only)"""
    ...
    if user:
        ...
        if user_id_check != str(MAIN_ADMIN_ID) and user_id_check not in admin_data.get("admins", []):
            await ctx.send(embed=create_error_embed("Access Denied", "Only ZXNODES admins can manage other users' VPS."))
            return
        
        user_id = str(user.id)
        vps_list = vps_data.get(user_id, [])
        if not vps_list:
            await ctx.send(embed=create_error_embed("No VPS Found", f"{user.mention} doesn't have any ZXNODES VPS."))
            return
        
        # Create admin view for managing another user's VPS
        view = ManageView(str(ctx.author.id), vps_list, is_admin=True, owner_id=user_id)
        await ctx.send(embed=create_info_embed(f"Managing {user.name}'s ZXNODES VPS", f"Managing VPS for {user.mention}"), view=view)
    else:
        # User managing their own VPS
        user_id = str(ctx.author.id)
        vps_list = vps_data.get(user_id, [])
        if not vps_list:
            embed = create_embed("No VPS Found", "You don't have any ZXNODES VPS. Contact an admin to create one.", 0xff3366)
            add_field(embed, "Quick Actions", "• `!manage` - Manage VPS\n• Contact ZXNODES admin for VPS creation", False)
            await ctx.send(embed=embed)
            return
...
async def list_all_vps(ctx):
    """List all ZXNODES VPS and user information (Admin only)"""
    total_vps = 0
    ...
    for user_id, vps_list in vps_data.items():
        try:
            ...
            # User summary
            user_summary.append(f"**{user.name}** ({user.mention}) - {user_vps_count} ZXNODES VPS ({user_running} running, {user_suspended} suspended)")
            ...
            vps_info.append(f"❓ Unknown User ({user_id}) - {len(vps_list)} ZXNODES VPS")
    
    # Create multiple embeds if needed to avoid character limit
    embeds = []
    
    # First embed with overview
    embed = create_embed("All ZXNODES VPS Information", "Complete overview of all ZXNODES VPS deployments and user statistics", 0x1a1a1a)
    add_field(embed, "System Overview", f"**Total Users:** {total_users}\n**Total VPS:** {total_vps}\n**Running:** {running_vps}\n**Stopped:** {stopped_vps}\n**Suspended:** {suspended_vps}", False)
    embeds.append(embed)
    
    # User summary embed
    if user_summary:
        embed = create_embed("ZXNODES User Summary", f"Summary of all users and their ZXNODES VPS", 0x1a1a1a)
    ...
    if vps_info:
        ...
        for i in range(0, len(vps_info), 15):
            ...
            embed = create_embed(f"ZXNODES VPS Details ({i+1}-{min(i+15, len(vps_info))})", "List of all ZXNODES VPS deployments", 0x1a1a1a)
...
async def manage_shared_vps(ctx, owner: discord.Member, vps_number: int):
    """Manage a shared ZXNODES VPS"""
    owner_id = str(owner.id)
    user_id = str(ctx.author.id)
    if owner_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[owner_id]):
        await ctx.send(embed=create_error_embed("Invalid VPS", "Invalid VPS number or owner doesn't have a ZXNODES VPS."))
        return
    vps = vps_data[owner_id][vps_number - 1]
    if user_id not in vps.get("shared_with", []):
        await ctx.send(embed=create_error_embed("Access Denied", "You do not have access to this ZXNODES VPS."))
        return
...
async def share_user(ctx, shared_user: discord.Member, vps_number: int):
    """Share ZXNODES VPS access with another user"""
    user_id = str(ctx.author.id)
    shared_user_id = str(shared_user.id)
    if user_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[user_id]):
        await ctx.send(embed=create_error_embed("Invalid VPS", "Invalid VPS number or you don't have a ZXNODES VPS."))
        return
    ...
    if shared_user_id in vps["shared_with"]:
        await ctx.send(embed=create_error_embed("Already Shared", f"{shared_user.mention} already has access to this ZXNODES VPS!"))
        return
    vps["shared_with"].append(shared_user_id)
    save_data()
    await ctx.send(embed=create_success_embed("VPS Shared", f"ZXNODES VPS #{vps_number} shared with {shared_user.mention}!"))
    try:
        await shared_user.send(embed=create_embed("ZXNODES VPS Access Granted", f"You have access to VPS #{vps_number} from {ctx.author.mention}. Use `!manage-shared {ctx.author.mention} {vps_number}`", 0x00ff88))
    except discord.Forbidden:
...
async def revoke_share(ctx, shared_user: discord.Member, vps_number: int):
    """Revoke shared ZXNODES VPS access"""
    user_id = str(ctx.author.id)
    shared_user_id = str(shared_user.id)
    if user_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[user_id]):
        await ctx.send(embed=create_error_embed("Invalid VPS", "Invalid VPS number or you don't have a ZXNODES VPS."))
    ...
    if shared_user_id not in vps["shared_with"]:
        await ctx.send(embed=create_error_embed("Not Shared", f"{shared_user.mention} doesn't have access to this ZXNODES VPS!"))
        return
    vps["shared_with"].remove(shared_user_id)
    save_data()
    await ctx.send(embed=create_success_embed("Access Revoked", f"Access to ZXNODES VPS #{vps_number} revoked from {shared_user.mention}!"))
    try:
        await shared_user.send(embed=create_embed("ZXNODES VPS Access Revoked", f"Your access to VPS #{vps_number} by {ctx.author.mention} has been revoked.", 0xff3366))
    except discord.Forbidden:
        await ctx.send(embed=create_info_embed("Notification Failed", f"Could not DM {shared_user.mention}"))
...
async def delete_vps(ctx, user: discord.Member, vps_number: int, *, reason: str = "No reason"):
    """Delete a user's ZXNODES VPS (Admin only)"""
    user_id = str(user.id)
    if user_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[user_id]):
        await ctx.send(embed=create_error_embed("Invalid VPS", "Invalid VPS number or user doesn't have a ZXNODES VPS."))
        return
    vps = vps_data[user_id][vps_number - 1]
    container_name = vps["container_name"]

    await ctx.send(embed=create_info_embed("Deleting ZXNODES VPS", f"Removing VPS #{vps_number}..."))
        ...
        if not vps_data[user_id]:
            ...
            if ctx.guild:
                ...
                if vps_role and vps_role in user.roles:
                    try:
                        await user.remove_roles(vps_role, reason="No ZXNODES VPS ownership")
                    except discord.Forbidden:
                        logger.warning(f"Failed to remove ZXNODES VPS role from {user.name}")
        save_data()

        embed = create_success_embed("ZXNODES VPS Deleted Successfully")
        add_field(embed, "Owner", user.mention, True)
...
async def add_resources(ctx, vps_id: str, ram: int = None, cpu: int = None, disk: int = None):
    """Add resources to a ZXNODES VPS (Admin only)"""
    ...
    if not found_vps:
        await ctx.send(embed=create_error_embed("VPS Not Found", f"No ZXNODES VPS found with ID: `{vps_id}`"))
        return
    
    was_running = found_vps.get('status') == 'running' and not found_vps.get('suspended', False)
    if was_running:
        await ctx.send(embed=create_info_embed("Stopping VPS", f"Stopping ZXNODES VPS `{vps_id}` to apply resource changes..."))
        try:
    ...
    try:
        ...
        embed = create_success_embed("Resources Added", f"Successfully added resources to ZXNODES VPS `{vps_id}`")
...
async def admin_add(ctx, user: discord.Member):
    """Add ZXNODES admin (Main admin only)"""
    user_id = str(user.id)
    if user_id == str(MAIN_ADMIN_ID):
        await ctx.send(embed=create_error_embed("Already Admin", "This user is already the main ZXNODES admin!"))
        return

    if user_id in admin_data.get("admins", []):
        await ctx.send(embed=create_error_embed("Already Admin", f"{user.mention} is already a ZXNODES admin!"))
        return
    ...
    save_data()
    await ctx.send(embed=create_success_embed("Admin Added", f"{user.mention} is now a ZXNODES admin!"))
    try:
        await user.send(embed=create_embed("🎉 ZXNODES Admin Role Granted", f"You are now a ZXNODES admin by {ctx.author.mention}", 0x00ff88))
    except discord.Forbidden:
...
async def admin_remove(ctx, user: discord.Member):
    """Remove ZXNODES admin (Main admin only)"""
    user_id = str(user.id)
    if user_id == str(MAIN_ADMIN_ID):
        await ctx.send(embed=create_error_embed("Cannot Remove", "You cannot remove the main ZXNODES admin!"))
        return

    if user_id not in admin_data.get("admins", []):
        await ctx.send(embed=create_error_embed("Not Admin", f"{user.mention} is not a ZXNODES admin!"))
        return

    admin_data["admins"].remove(user_id)
    save_data()
    await ctx.send(embed=create_success_embed("Admin Removed", f"{user.mention} is no longer a ZXNODES admin!"))
    try:
        await user.send(embed=create_embed("⚠️ ZXNODES Admin Role Revoked", f"Your admin role was removed by {ctx.author.mention}", 0xff3366))
    except discord.Forbidden:
        await ctx.send(embed=create_info_embed("Notification Failed", f"Could not DM {user.mention}"))
...
async def admin_list(ctx):
    """List all ZXNODES admins (Main admin only)"""
    admins = admin_data.get("admins", [])
    main_admin = await bot.fetch_user(MAIN_ADMIN_ID)

    embed = create_embed("👑 ZXNODES Admin Team", "Current ZXNODES administrators:", 0x1a1a1a)
    ...
    if admins:
        ...
        add_field(embed, "🛡️ Admins", "No additional ZXNODES admins", False)
...
async def user_info(ctx, user: discord.Member):
    """Get detailed information about a ZXNODES user (Admin only)"""
    user_id = str(user.id)

    # Get user's VPS
    vps_list = vps_data.get(user_id, [])

    embed = create_embed(f"ZXNODES User Information - {user.name}", f"Detailed information for {user.mention}", 0x1a1a1a)
    ...
    if vps_list:
        ...
        add_field(embed, "🖥️ ZXNODES VPS Information", vps_summary, False)
        ...
        if len(vps_info) > 10:
            ...
            for i in range(10, len(vps_info), 10):
                ...
                additional_embed = create_embed(f"ZXNODES VPS List ({i+1}-{min(i+10, len(vps_info))})", f"More VPS for {user.mention}", 0x1a1a1a)
        ...
        add_field(embed, "🖥️ ZXNODES VPS Information", "**No VPS owned**", False)
        await ctx.send(embed=embed)

    # Check if user is admin
    is_admin_user = user_id == str(MAIN_ADMIN_ID) or user_id in admin_data.get("admins", [])
    add_field(embed, "🛡️ ZXNODES Admin Status", f"**{'Yes' if is_admin_user else 'No'}**", False)

@bot.command(name='serverstats')
@is_admin()
async def server_stats(ctx):
    """Show ZXNODES server statistics (Admin only)"""
    total_users = len(vps_data)
    ...
    embed = create_embed("📊 ZXNODES Server Statistics", "Current ZXNODES server overview", 0x1a1a1a)
...
async def vps_info(ctx, container_name: str = None):
    """Get detailed ZXNODES VPS information (Admin only)"""
    if not container_name:
        ...
        for user_id, vps_list in vps_data.items():
            try:
                ...
                for i, vps in enumerate(vps_list):
                    ...
                    if vps.get('suspended', False):
                        status_text += " (SUSPENDED)"
                    all_vps.append(f"**{user.name}** - ZXNODES VPS {i+1}: `{vps['container_name']}` - {status_text}")
            except:
                pass
        ...
        for i in range(0, len(all_vps), 20):
            ...
            embed = create_embed(f"🖥️ All ZXNODES VPS ({i+1}-{min(i+20, len(all_vps))})", f"List of all ZXNODES VPS deployments", 0x1a1a1a)
            add_field(embed, "VPS List", "\n".join(chunk), False)
            await ctx.send(embed=embed)
    else:
        ...
        if not found_vps:
            await ctx.send(embed=create_error_embed("VPS Not Found", f"No ZXNODES VPS found with container name: `{container_name}`"))
            return

        suspended_text = " (SUSPENDED)" if found_vps.get('suspended', False) else ""
        embed = create_embed(f"🖥️ ZXNODES VPS Information - {container_name}", f"Details for VPS owned by {found_user.mention}{suspended_text}", 0x1a1a1a)
...
async def restart_vps(ctx, container_name: str):
    """Restart a ZXNODES VPS (Admin only)"""
    await ctx.send(embed=create_info_embed("Restarting VPS", f"Restarting ZXNODES VPS `{container_name}`..."))
    ...
    try:
        ...
        await ctx.send(embed=create_success_embed("VPS Restarted", f"ZXNODES VPS `{container_name}` has been restarted successfully!"))
...
async def backup_vps(ctx, container_name: str):
    """Create a snapshot of a ZXNODES VPS (Admin only)"""
    snapshot_name = f"zxnodes-{container_name}-backup-{datetime.now().strftime('%Y%m%d-%H%M%S')}"

    await ctx.send(embed=create_info_embed("Creating ZXNODES Backup", f"Creating snapshot of `{container_name}`..."))

    try:
        await execute_lxc(f"lxc snapshot {container_name} {snapshot_name}")
        await ctx.send(embed=create_success_embed("Backup Created", f"ZXNODES Snapshot `{snapshot_name}` created successfully!"))
...
async def restore_vps(ctx, container_name: str, snapshot_name: str):
    """Restore a ZXNODES VPS from snapshot (Admin only)"""
    await ctx.send(embed=create_info_embed("Restoring VPS", f"Restoring `{container_name}` from ZXNODES snapshot `{snapshot_name}`..."))

    try:
        await execute_lxc(f"lxc restore {container_name} {snapshot_name}")
        await ctx.send(embed=create_success_embed("VPS Restored", f"ZXNODES VPS `{container_name}` has been restored from snapshot!"))
...
async def list_snapshots(ctx, container_name: str):
    """List all snapshots for a ZXNODES VPS (Admin only)"""
    try:
        ...
        if snapshots:
            ...
            for i in range(0, len(snapshots), 20):
                ...
                embed = create_embed(f"📸 ZXNODES Snapshots for {container_name} ({i+1}-{min(i+20, len(snapshots))})", f"List of snapshots", 0x1a1a1a)
                add_field(embed, "Snapshots", "\n".join([f"• {snap}" for snap in chunk]), False)
                await ctx.send(embed=embed)
        else:
            await ctx.send(embed=create_info_embed("No Snapshots", f"No ZXNODES snapshots found for `{container_name}`"))
...
async def execute_command(ctx, container_name: str, *, command: str):
    """Execute a command inside a ZXNODES VPS (Admin only)"""
    await ctx.send(embed=create_info_embed("Executing Command", f"Running command in ZXNODES VPS `{container_name}`..."))
...
async def stop_all_vps(ctx):
    """Stop all ZXNODES VPS using lxc stop --all --force (Admin only)"""
    await ctx.send(embed=create_warning_embed("Stopping All ZXNODES VPS", "⚠️ **WARNING:** This will stop ALL running VPS on the ZXNODES server.\n\nThis action cannot be undone. Continue?"))
    ...
    class ConfirmView(discord.ui.View):
        ...
        async def confirm(self, interaction: discord.Interaction, item: discord.ui.Button):
            ...
            try:
                ...
                if proc.returncode == 0:
                    ...
                    embed = create_success_embed("All ZXNODES VPS Stopped", f"Successfully stopped {stopped_count} VPS using `lxc stop --all --force`")
                    output_text = stdout.decode() if stdout else 'No output'
                    add_field(embed, "Command Output", f"```\n{output_text}\n```", False)
                    await interaction.followup.send(embed=embed)
                else:
                    error_msg = stderr.decode() if stderr else "Unknown error"
                    embed = create_error_embed("Stop Failed", f"Failed to stop ZXNODES VPS: {error_msg}")
                    await interaction.followup.send(embed=embed)
        ...
        @discord.ui.button(label="Cancel", style=discord.ButtonStyle.secondary)
        async def cancel(self, interaction: discord.Interaction, item: discord.ui.Button):
            await interaction.response.edit_message(embed=create_info_embed("Operation Cancelled", "The stop all ZXNODES VPS operation has been cancelled."))
...
async def cpu_monitor_control(ctx, action: str = "status"):
    """Control ZXNODES CPU monitoring system (Admin only)"""
    global cpu_monitor_active
    
    if action.lower() == "status":
        status = "Active" if cpu_monitor_active else "Inactive"
        embed = create_embed("ZXNODES CPU Monitor Status", f"ZXNODES CPU monitoring is currently **{status}**", 0x00ccff if cpu_monitor_active else 0xffaa00)
        add_field(embed, "Threshold", f"{CPU_THRESHOLD}% CPU usage", True)
        add_field(embed, "Check Interval", f"60 seconds (host)", True)
        await ctx.send(embed=embed)
    elif action.lower() == "enable":
        cpu_monitor_active = True
        await ctx.send(embed=create_success_embed("CPU Monitor Enabled", "ZXNODES CPU monitoring has been enabled."))
    elif action.lower() == "disable":
        cpu_monitor_active = False
        await ctx.send(embed=create_warning_embed("CPU Monitor Disabled", "ZXNODES CPU monitoring has been disabled."))
    else:
...
async def resize_vps(ctx, container_name: str, ram: int = None, cpu: int = None, disk: int = None):
    """Resize ZXNODES VPS resources (Admin only)"""
    if ram is None and cpu is None and disk is None:
    ...
    was_running = found_vps.get('status') == 'running' and not found_vps.get('suspended', False)
    if was_running:
        await ctx.send(embed=create_info_embed("Stopping VPS", f"Stopping ZXNODES VPS `{container_name}` to apply resource changes..."))
        try:
    ...
    try:
        ...
        embed = create_success_embed("VPS Resized", f"Successfully resized resources for ZXNODES VPS `{container_name}`")
        add_field(embed, "Changes Applied", "\n".join(changes), False)
        await ctx.send(embed=embed)
...
async def clone_vps(ctx, container_name: str, new_name: str = None):
    """Clone a ZXNODES VPS (Admin only)"""
    if not new_name:
        # Generate a new name if not provided
        timestamp = datetime.now().strftime('%Y%m%d-%H%M%S')
        new_name = f"zxnodes-{container_name}-clone-{timestamp}"
    
    await ctx.send(embed=create_info_embed("Cloning VPS", f"Cloning ZXNODES VPS `{container_name}` to `{new_name}`..."))
    ...
    try:
        ...
        embed = create_success_embed("VPS Cloned", f"Successfully cloned ZXNODES VPS `{container_name}` to `{new_name}`")
...
async def migrate_vps(ctx, container_name: str, target_pool: str):
    """Migrate a ZXNODES VPS to a different storage pool (Admin only)"""
    await ctx.send(embed=create_info_embed("Migrating VPS", f"Migrating ZXNODES VPS `{container_name}` to storage pool `{target_pool}`..."))
    ...
    try:
        ...
        temp_name = f"zxnodes-{container_name}-temp-{int(time.time())}"
        ...
        await ctx.send(embed=create_success_embed("VPS Migrated", f"Successfully migrated ZXNODES VPS `{container_name}` to storage pool `{target_pool}`"))
...
async def vps_stats(ctx, container_name: str):
    """Show detailed resource usage statistics for a ZXNODES VPS (Admin only)"""
    await ctx.send(embed=create_info_embed("Gathering Statistics", f"Collecting statistics for ZXNODES VPS `{container_name}`..."))
    ...
    try:
        ...
        embed = create_embed(f"📊 ZXNODES VPS Statistics - {container_name}", f"Resource usage statistics", 0x1a1a1a)
...
async def vps_network(ctx, container_name: str, action: str, value: str = None):
    """Manage ZXNODES VPS network settings (Admin only)"""
    ...
    try:
        if action.lower() == "list":
            ...
            if proc.returncode == 0:
                ...
                embed = create_embed(f"🌐 ZXNODES Network Interfaces - {container_name}", "Network configuration", 0x1a1a1a)
            ...
            await ctx.send(embed=create_success_embed("Network Limited", f"Set ZXNODES network limit to {value} for `{container_name}`"))
        
        elif action.lower() in ["add", "remove"]:
            await ctx.send(embed=create_info_embed("Not Implemented", f"ZXNODES Network {action} is not yet implemented. Use list or limit for now."))
...
async def vps_processes(ctx, container_name: str):
    """Show running processes in a ZXNODES VPS (Admin only)"""
    await ctx.send(embed=create_info_embed("Gathering Processes", f"Listing processes in ZXNODES VPS `{container_name}`..."))
    ...
    try:
        ...
        if proc.returncode == 0:
            ...
            embed = create_embed(f"⚙️ ZXNODES Processes - {container_name}", "Running processes", 0x1a1a1a)
...
async def vps_logs(ctx, container_name: str, lines: int = 50):
    """Show recent logs from a ZXNODES VPS (Admin only)"""
    await ctx.send(embed=create_info_embed("Gathering Logs", f"Fetching last {lines} lines from ZXNODES VPS `{container_name}`..."))
    ...
    try:
        ...
        if proc.returncode == 0:
            ...
            embed = create_embed(f"📋 ZXNODES Logs - {container_name}", f"Last {lines} log lines", 0x1a1a1a)
...
async def suspend_vps(ctx, container_name: str, *, reason: str = "Admin action"):
    """Suspend a ZXNODES VPS (Admin only)"""
    found = False
    for uid, lst in vps_data.items():
        for vps in lst:
            if vps['container_name'] == container_name:
                if vps.get('status') != 'running':
                    await ctx.send(embed=create_error_embed("Cannot Suspend", "ZXNODES VPS must be running to suspend."))
                    return
                try:
                    await execute_lxc(f"lxc stop {container_name}")
                    vps['status'] = 'suspended'
                ...
                try:
                    ...
                    embed = create_warning_embed("🚨 ZXNODES VPS Suspended", f"Your VPS `{container_name}` has been suspended by an admin.\n\n**Reason:** {reason}\n\nContact a ZXNODES admin to unsuspend.")
                    await owner.send(embed=embed)
                except Exception as dm_e:
                    logger.error(f"Failed to DM owner {uid}: {dm_e}")
                await ctx.send(embed=create_success_embed("VPS Suspended", f"ZXNODES VPS `{container_name}` suspended. Reason: {reason}"))
                found = True
                break
        if found:
            break
    if not found:
        await ctx.send(embed=create_error_embed("Not Found", f"ZXNODES VPS `{container_name}` not found."))

@bot.command(name='unsuspend-vps')
@is_admin()
async def unsuspend_vps(ctx, container_name: str):
    """Unsuspend a ZXNODES VPS (Admin only)"""
    found = False
    for uid, lst in vps_data.items():
        for vps in lst:
            if vps['container_name'] == container_name:
                if not vps.get('suspended', False):
                    await ctx.send(embed=create_error_embed("Not Suspended", "ZXNODES VPS is not suspended."))
                    return
                try:
                    vps['suspended'] = False
                    vps['status'] = 'running'
                    await execute_lxc(f"lxc start {container_name}")
                    save_data()
                    await ctx.send(embed=create_success_embed("VPS Unsuspended", f"ZXNODES VPS `{container_name}` unsuspended and started."))
                    found = True
                except Exception as e:
...
async def suspension_logs(ctx, container_name: str = None):
    """View ZXNODES suspension logs (Admin only)"""
    if container_name:
        # Specific VPS
        found = None
        ...
        if not history:
            await ctx.send(embed=create_info_embed("No Suspensions", f"No ZXNODES suspension history for `{container_name}`."))
            return
        embed = create_embed("ZXNODES Suspension History", f"For `{container_name}`")
        text = []
        ...
        if not all_logs:
            await ctx.send(embed=create_info_embed("No Suspensions", "No ZXNODES suspension events recorded."))
            return
        # Split into embeds
        for i in range(0, len(all_logs), 10):
            chunk = all_logs[i:i+10]
            embed = create_embed(f"ZXNODES Suspension Logs ({i+1}-{min(i+10, len(all_logs))})", f"Global suspension events (newest first)")
            add_field(embed, "Events", "\n".join(chunk), False)
            await ctx.send(embed=embed)

@bot.command(name='help')
async def show_help(ctx):
    """Show ZXNODES help information"""
    user_id = str(ctx.author.id)
    ...
    embed = create_embed("📚 ZXNODES Command Help - User Commands", "ZXNODES VPS Manager Commands:", 0x1a1a1a)

    user_commands = [
        ("!ping", "Check ZXNODES bot latency"),
        ("!uptime", "Show host uptime"),
        ("!myvps", "List your ZXNODES VPS"),
        ("!manage [@user]", "Manage your VPS or another user's VPS (Admin only)"),
        ("!share-user @user <vps_number>", "Share ZXNODES VPS access"),
        ("!share-ruser @user <vps_number>", "Revoke ZXNODES VPS access"),
        ("!manage-shared @owner <vps_number>", "Manage shared ZXNODES VPS")
    ]
    ...
    if is_user_admin:
        ...
        embed = create_embed("📚 ZXNODES Command Help - Admin Commands", "ZXNODES VPS Manager Commands:", 0x1a1a1a)

        admin_commands = [
            ("!lxc-list", "List all LXC containers"),
            ("!create <ram_gb> <cpu_cores> <disk_gb> @user", "Create custom ZXNODES VPS"),
            ("!delete-vps @user <vps_number> <reason>", "Delete user's ZXNODES VPS"),
            ("!add-resources <vps_id> [ram] [cpu] [disk]", "Add resources to a ZXNODES VPS"),
            ("!resize-vps <container> [ram] [cpu] [disk]", "Resize ZXNODES VPS resources"),
            ("!suspend-vps <container> [reason]", "Suspend a ZXNODES VPS"),
            ("!unsuspend-vps <container>", "Unsuspend a ZXNODES VPS"),
            ("!suspension-logs [container]", "View ZXNODES suspension logs"),
            ("!userinfo @user", "Get detailed ZXNODES user information"),
            ("!serverstats", "Show ZXNODES server statistics"),
            ("!vpsinfo [container]", "Get ZXNODES VPS information"),
            ("!list-all", "View all ZXNODES VPS and user information"),
            ("!restart-vps <container>", "Restart a ZXNODES VPS"),
            ("!backup-vps <container>", "Create ZXNODES VPS snapshot"),
            ("!restore-vps <container> <snapshot>", "Restore from ZXNODES snapshot"),
            ("!list-snapshots <container>", "List ZXNODES VPS snapshots"),
            ("!exec <container> <command>", "Execute command in ZXNODES VPS"),
            ("!stop-vps-all", "Stop all ZXNODES VPS with lxc stop --all --force"),
            ("!cpu-monitor <status|enable|disable>", "Control ZXNODES CPU monitoring system"),
            ("!clone-vps <container> [new_name]", "Clone a ZXNODES VPS"),
            ("!migrate-vps <container> <pool>", "Migrate ZXNODES VPS to storage pool"),
            ("!vps-stats <container>", "Show ZXNODES VPS resource stats"),
            ("!vps-network <container> <action> [value]", "Manage ZXNODES network"),
            ("!vps-processes <container>", "List ZXNODES processes"),
            ("!vps-logs <container> [lines]", "Show ZXNODES system logs")
        ]
    ...
    if is_user_main_admin:
        ...
        embed = create_embed("📚 ZXNODES Command Help - Main Admin Commands", "ZXNODES VPS Manager Commands:", 0x1a1a1a)

        main_admin_commands = [
            ("!admin-add @user", "Promote to ZXNODES admin"),
            ("!admin-remove @user", "Remove ZXNODES admin"),
            ("!admin-list", "View all ZXNODES admins")
        ]
        ...
        embed.set_footer(text="ZXNODES VPS Manager • Auto-suspend on high usage • Enhanced monitoring")
        await ctx.send(embed=embed)
...
async def manage_typo(ctx):
    ...
    await ctx.send(embed=create_info_embed("Command Correction", "Did you mean `!manage`? Use the correct ZXNODES command."))
...
async def stats_alias(ctx):
    ...
    if str(ctx.author.id) == str(MAIN_ADMIN_ID) or str(ctx.author.id) in admin_data.get("admins", []):
        ...
        await ctx.send(embed=create_error_embed("Access Denied", "This ZXNODES command requires admin privileges."))
